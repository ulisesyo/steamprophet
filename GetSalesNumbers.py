    ...
    for g in gamesId:
        ...
        if (data['appid'] == 999999):
            ...
            try:
                ...
                f.write(str(g) + "\t" + str(price) + "\t" + str(data['players_forever']) + "\t" + str(data['players_forever_variance']) + "\t" + str(minPlayers) + "\tError at steamdb\n")
                continue

            if price > 0:
                estimation = float(priceAndDiscount[0])*int(minPlayers)
                print("Lower price: " + str(priceAndDiscount[0]) + "->" + str(estimation))
                f.write(str(g) + "\t" + str(price) + "\t" + str(data['players_forever']) + "\t" + str(data['players_forever_variance']) + "\t" + str(minPlayers) + "\t" + str(priceAndDiscount[0]) + "\t" + str(estimation) + "\n")
            else:
                f.write(str(g) + "\tF2P\n")
        idx = idx + 1
        time.sleep(0.25)
